...
namespace ros2
{
	...
	void PCLUtils::doTransform(const pcl::PointCloud<pcl::PointXYZ> &src, pcl::PointCloud<pcl::PointXYZ> &dst,
							   const geometry_msgs::msg::TransformStamped &transform)
	{
		dst.header = src.header;
		dst.header.frame_id = transform.header.frame_id;
		...
		{
			doTransform(src.points[i], dst.points[i], transform);
		}
	}

	void PCLUtils::doTransform(const pcl::PointCloud<pcl::PointXYZ>::Ptr &src, pcl::PointCloud<pcl::PointXYZ>::Ptr &dst,
							   const geometry_msgs::msg::TransformStamped &transform)
	{
		dst->header = src->header;
		dst->header.frame_id = transform.header.frame_id;
