...
namespace ros2
{
  ...
  void Rotate::rotate(float &x, float &y, float &z, const geometry_msgs::msg::Quaternion &q_rot)
  {
    float old_x = x;
    float old_y = y;
    float old_z = z;
  ...
  void Rotate::rotate(pcl::PointXYZTIRRRA &p, const geometry_msgs::msg::Quaternion &q_rot)
  {
    Rotate::rotate(p.x, p.y, p.z, q_rot);
  }
