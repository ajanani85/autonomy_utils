...
#include <autonomy_utils/lib/ros2_utility.h>
...
pcl::PointCloud<pcl::PointXYZ>::ConstPtr AMPCLUtils::convertToPointXYZ(const sensor_msgs::msg::PointCloud2::SharedPtr pc2,
...
void AMPCLUtils::convertToPointXYZIR(const sensor_msgs::msg::PointCloud2::SharedPtr pc2,
...
void AMPCLUtils::convertToPointXYZIR(const sensor_msgs::msg::PointCloud2 &pc2,
          pcl::PointCloud<pcl::PointXYZIR> &pcl)
{
	pcl.header = pcl_conversions::toPCL(pc2.header);
	pcl.resize(pc2.height * pc2.width);
...
void AMPCLUtils::convertToPointXYZ(const sensor_msgs::msg::PointCloud2::SharedPtr pc2,
...
void AMPCLUtils::convertToPointXYZ(const sensor_msgs::msg::PointCloud2 &pc2,
           pcl::PointCloud<pcl::PointXYZ> &pcl)
{
...
bool AMPCLUtils::convertToPointXYZ(const sensor_msgs::msg::Image::ConstSharedPtr &image, const sensor_msgs::msg::CameraInfo::SharedPtr &cam_info,
            ...
            double downsample_factor, double depth_factor, double min_distance, double max_distance)
{
	...
	ros2::AMPCLUtils::setNaN(bad_point);
	...
	for(int r = 0; r < frame.rows; r += downsample_factor)
	{
		for(int c = 0; c < frame.cols; c += downsample_factor)
		{
			...
			ros2::AMPCLUtils::doTransform(p, p,transform);
			cloud.points[cnt] = p;
		}
	}
...
void AMPCLUtils::toPointCloud2(const pcl::PointCloud<pcl::PointXYZ> &cloud, sensor_msgs::msg::PointCloud2 &cloud_out)
{
	pcl::toROSMsg<pcl::PointXYZ>(cloud, cloud_out);
}

void AMPCLUtils::toPointCloud2(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, sensor_msgs::msg::PointCloud2 &cloud_out)
{
	pcl::toROSMsg<pcl::PointXYZRGB>(cloud, cloud_out);
}

void AMPCLUtils::toPointCloud2(const pcl::PointCloud<pcl::PointXYZIR> &cloud, sensor_msgs::msg::PointCloud2 &cloud_out)
{
	pcl::toROSMsg<pcl::PointXYZIR>(cloud, cloud_out);
}

void AMPCLUtils::toPointCloud2(const pcl::PointCloud<pcl::PointXYZIR>::Ptr cloud,
...
void AMPCLUtils::toPointCloud2(const pcl::PointCloud<pcl::PointXYZIR> &cloud,
		  const std::vector<int> &valid_indices, sensor_msgs::msg::PointCloud2 &cloud_out)
{
	pcl::PointCloud<pcl::PointXYZIR> cloud_in;
	cloud_in.points.resize(cloud.size());
...
bool AMPCLUtils::isSetToNaN(pcl::PointXYZ &point)
{
  if (std::isnan(point.x) || std::isnan(point.y) || std::isnan(point.z))
  {
    setNaN(point);
...
bool AMPCLUtils::isSetToNaN(pcl::PointXYZRGB &point)
{
  if (std::isnan(point.x) || std::isnan(point.y) || std::isnan(point.z))
  {
    setNaN(point);
...
void AMPCLUtils::setNaN(pcl::PointXYZ &point)
{
  point.x = std::nanf("");
  point.y = std::nanf("");
  point.z = std::nanf("");
...
void AMPCLUtils::doTransform(const pcl::PointXYZ & t_in, pcl::PointXYZ & t_out, const geometry_msgs::msg::TransformStamped & transform)
{
...
void AMPCLUtils::doTransform(const pcl::PointXYZIR & t_in, pcl::PointXYZIR & t_out, const geometry_msgs::msg::TransformStamped & transform)
{
	KDL::Vector v_out = tf2::gmTransformToKDL(transform) * KDL::Vector(t_in.x, t_in.y, t_in.z);
	t_out.x = v_out[0];
	t_out.y = v_out[1];
...
void AMPCLUtils::setNaN(pcl::PointXYZRGB &point)
{
  point.x = std::nanf("");
  point.y = std::nanf("");
  point.z = std::nanf("");
